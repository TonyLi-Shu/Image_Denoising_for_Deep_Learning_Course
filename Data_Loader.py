...
class Images_Dataset_folder(torch.utils.data.Dataset):
    ...
    def __init__(self, images_dir, labels_dir,transformI = None, transformM = None):
        ...
        self.transformM = transformM
        """
        NEED to change the input width and height here!
        """
        self.inputW = 256
        self.inputH = 256
        if self.transformI:
            ...
            self.tx = torchvision.transforms.Compose([
                # torchvision.transforms.Resize((self.inputW,self.inputH)),
    ...
    def __getitem__(self, i):
        ...
        seed= np.random.randint(0, 2 ** 32 - 2, dtype=np.int64) # make a seed with numpy generator
